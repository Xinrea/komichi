...
- `folia.wasm`：渲染引擎，内嵌三套主题的 JSON 和 GLSL ES shader。
...
本站 `music.js` 负责音频、播放控件、音量记忆及渲染调度。主题在 WASM 虚拟文件系统 `/folia/themes/` 内，通过 `folia_load` 加载；浏览器仅向对应目录提供共享的 CJK 字体。每首歌随机选择主题，同一次访问中保持不变。修改本站曲库和歌词不需要重新构建引擎。

## 验证

启动本站 `python3 scripts/serve.py` 后，使用安装了 Playwright 的 Node 环境运行 `node scripts/verify-music.cjs`。检查包括三套嵌入主题、WebGL 帧缓冲、CJK 字形、歌词跳转和间奏、播放模式、音量记忆、歌曲信息，以及本地导入和来源入口确实已移除。
